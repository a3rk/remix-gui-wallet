                                    ...
                                    const QString &language, bool testnet)
{
    ...
    RemixCoin::Wallet * w = m_pimpl->createWallet(path.toStdString(), password.toStdString(),
...
Wallet *WalletManager::openWallet(const QString &path, const QString &password, bool testnet)
{
    ...
    qDebug("%s: opening wallet at %s, testnet = %d ",
           __FUNCTION__, qPrintable(path), testnet);

    RemixCoin::Wallet * w =  m_pimpl->openWallet(path.toStdString(), password.toStdString(), testnet);
    qDebug("%s: opened wallet: %s, status: %d", __FUNCTION__, w->address().c_str(), w->status());
    m_currentWallet  = new Wallet(w);
...
Wallet *WalletManager::recoveryWallet(const QString &path, const QString &memo, bool testnet, quint64 restoreHeight)
{
    ...
    RemixCoin::Wallet * w = m_pimpl->recoveryWallet(path.toStdString(), memo.toStdString(), testnet, restoreHeight);
    m_currentWallet = new Wallet(w);
    return m_currentWallet;
}
                                            ...
                                            quint64 restoreHeight)
{
    ...
    RemixCoin::Wallet * w = m_pimpl->createWalletFromKeys(path.toStdString(), language.toStdString(), testnet, restoreHeight,
...
{
    return RemixCoin::Wallet::maximumAllowedAmount();
}
...
{
    return QString::fromStdString(RemixCoin::Wallet::displayAmount(amount));
}

quint64 WalletManager::amountFromString(const QString &amount) const
{
    return RemixCoin::Wallet::amountFromString(amount.toStdString());
}

quint64 WalletManager::amountFromDouble(double amount) const
{
    return RemixCoin::Wallet::amountFromDouble(amount);
}

bool WalletManager::paymentIdValid(const QString &payment_id) const
{
    return RemixCoin::Wallet::paymentIdValid(payment_id.toStdString());
}

bool WalletManager::addressValid(const QString &address, bool testnet) const
{
    return RemixCoin::Wallet::addressValid(address.toStdString(), testnet);
}

bool WalletManager::keyValid(const QString &key, const QString &address, bool isViewKey,  bool testnet) const
{
    std::string error;
    if(!RemixCoin::Wallet::keyValid(key.toStdString(), address.toStdString(), isViewKey, testnet, error)){
...
{
    return QString::fromStdString(RemixCoin::Wallet::paymentIdFromAddress(address.toStdString(), testnet));
}
...
{
    RemixCoin::WalletManagerFactory::setLogLevel(logLevel);
}

void WalletManager::setLogCategories(const QString &categories)
{
    RemixCoin::WalletManagerFactory::setLogCategories(categories.toStdString());
}
...
//  const std::tuple<bool, std::string, std::string, std::string, std::string> result = RemixCoin::WalletManager::checkUpdates(software.toStdString(), subdir.toStdString());
//  if (!std::get<0>(result))
...
{
    m_pimpl =  RemixCoin::WalletManagerFactory::getWalletManager();
}
